...
// Updated for tax year 2025 using IRS 2025 thresholds and OBBB-adjusted standard deduction.
const US_STANDARD_DEDUCTION_2025 = {
  single: 15750,
  married_joint: 31500,
...
export function calculateUSTax({
  ...
  const status = US_BRACKETS_2025[filingStatus] ? filingStatus : 'single';
  const gross = max0(grossIncome);
  const adjustments = max0(D(aboveLineDeductions).plus(pretaxContributions));
  const adjustedIncome = max0(gross.minus(adjustments));

  const standardDeduction = D(US_STANDARD_DEDUCTION_2025[status]);
  ...
  const bracketResult = progressiveTax(taxableIncome, US_BRACKETS_2025[status]);
